...
def _chuva_7dias_uma_vez():
    ...
    for i, data in enumerate(datas):
        chuvas = valores_dos_modelos(diario, "precipitation_sum", i)
        if not chuvas:
            break
        probs = valores_dos_modelos(diario, "precipitation_probability_max", i)
        saida.append({
            "data": data,
            "chuva_mm": round(mediana(chuvas), 1),
            "probabilidade_pct": int(round(mediana(probs))) if probs else 0,
            "codigo_tempo": codigo_do_tempo(diario, i),
            "modelos": len(chuvas),
